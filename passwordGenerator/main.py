...
def generatePassword():
    lengthOfPassword = entry.get()
    result_label.config(text = "Enter a Valid Length") if int(lengthOfPassword) < 1 else result_label.config(text=f"Your Password is: \n {password(int(lengthOfPassword))}")
...
result_label = tk.Label(root, text="", font=("Arial", 24), bg="black", fg="white")
